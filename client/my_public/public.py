...
print(Config_Impl.Protocol_Len,type(Config_Impl.Protocol_Len), Config_Impl.Data_Len, type( Config_Impl.Data_Len))
...
class Protocol:
    ...
    Response_Os_Error = 2005

    OsError = 3000
    OsRemoveItem = 3002
    OsMkDir = 3004
    OsReName = 3005
    Write_Error_Cancel_Download = 3006
    Write_Error_Cancel_Download_Response = 3007

    Write_Error_Cancel_Upload = 3008
    Write_Error_Cancel_Upload_Response = 3009
    WriteEndFlag = b'~*~*'
    GetSysInfo = 4001
...
class Tools:
    ...
    def encode(protocol, data=b""):
        ...
        protocol.update(dict(len=len(data)))
...
class WriteFile:
    def __init__(self, *args):
        ...
        self.before_first_write, self.every_write, self.after_end, self.data_socket = args
        self.File_Fp = None
        self._Stop_Write = False
        # self.IO_Error = False
        # self.IO_Error_Msg = str()
        self.Update_Widget_Flag = False
    ...
    def write(self, data):
        ...
        try:
            ...
            if self.File_Fp:
                ...
                if self.every_write:
                    ...
                    if self.every_write and UPDATE_INTERVAL:
                        if int(time.time()) % 2 == 0 and not self.Update_Widget_Flag:
                            self.Update_Widget_Flag = True
                            self.every_write.emit(progress_data)
                        if int(time.time()) % 2 == 1 and self.Update_Widget_Flag:
                            self.Update_Widget_Flag = False
                            self.every_write.emit(progress_data)
                    else:
                        self.every_write.emit(progress_data)
            ...
            # 文件首次写入
                ...
                abs_path = os.path.join(data["write_path"], data["file_name"])
                ...
                self.File_Fp.write(stream)
                # 首次写入回调
                # self.before_first_write and self.before_first_write.emit(dict(is_local=data["is_local"]))
                if data["last_group"]:
                    # 写入结束回调
...
class SendFile:
    def __init__(self, Conn, update_widget=False, *args):
        ...
        self.before_begin_send, self.every_send, self.after_end, self.data_socket = args
        self.Conn = Conn
